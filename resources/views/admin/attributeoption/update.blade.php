...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="row">
            <form action="{{ route('admin.attributeoption.update', $option->id) }}" method="POST">
                @csrf
                <div class="col-xl-12">
                                ...
                                <div class="tab-pane fade {{ $loop->first ? 'active show' : '' }}" id="{{ $key }}"
                                    ...
                                    <div class="card-body">
                                        <div class="form-group">
                                            ...
                                            <input type="hidden" name="attribute_id" value="{{ $option->attribute->id }}">
                                            <input class="form-control" id="name" type="text"
                                                name="name[{{ $key }}]" disabled
                                                value="{{ old('name' . $key, $option->attribute->getTranslation('name', $key)) }}">
                                            <label class="mb-1" for="name">Atribut Option Adı</label>
                                            <input class="form-control" id="name" type="text"
                                                name="name[{{ $key }}]" placeholder="Enter Name"
                                                value="{{ old('name' . $key, $option->getTranslation('name', $key)) }}">
                                        </div>
                                    </div>
                                </div>
                            @endforeach
