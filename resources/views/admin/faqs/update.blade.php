...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Faq /</span> Yarat</h4>

        <div class="row">
            <form action="{{ route('admin.faqs.update', $faq->id) }}" method="POST">
                @csrf
                <div class="col-xl-12">
                    <h5 class="text-muted">Faqlar</h5>
                                    ...
                                    <div class="card-body">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" type="text" name="title[{{ $key }}]"
                                                id="title"
                                                value="{{ old('title' . $key, $faq->getTranslation('title', $key)) }}"
                                                placeholder="Enter Title">
                                        </div>
                                        <div class="form-group">
                                            <label class="mb-1" for="desc">Description</label>
                                            <textarea placeholder="Enter Description" id="editor" name="desc[{{ $key }}]" class="mt-3 mb-3">{{ old('desc' . $key, $faq->getTranslation('desc', $key)) }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
