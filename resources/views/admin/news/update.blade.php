...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Xəbər/</span> Düzənləmə</h4>
                                ...
                                <div class="tab-pane fade {{ $loop->first ? 'active show' : '' }}" id="{{ $key }}"
                                    ...
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label class="mb-1">Xəbər Başlıq</label>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea placeholder="Mündəricat Yazın" id="editor" class="form-control" name="desc[{{ $key }}]" class="mt-3 mb-3">{{ old('desc' . $key, $news->getTranslation('desc', $key)) }}</textarea>
                                        </div>
                                        <div class="form-group">
                                            <label for="slug" class="mb-1">Slug</label>
                                            <input type="text"
                                                value="{{ old('slug' . $key, $news->getTranslation('slug', $key)) }}"
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <select id="select2" class="js-example-basic-multiple form-control" name="tag_id[]"
                                        ...
                                        @foreach ($tags as $tag)
                                            <option
                                                {{ in_array($tag->id,optional($news->tags)->pluck('id')->all() ?? [])? 'selected': '' }}
                                                value="{{ $tag->id }}">
