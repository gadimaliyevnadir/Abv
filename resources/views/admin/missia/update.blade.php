...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Missiya /</span> Düzənləmə</h4>

        <div class="row">
            <form action="{{ route('admin.missia.update',$missium->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="col-xl-12">
                                    ...
                                    <div class="card-body">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" type="text" name="title[{{ $key }}]"
                                                id="title" value="{{ old('title' . $key, $missium->getTranslation('title', $key)) }}" placeholder="Başlıq Yazın">
                                        </div>
                                        <div class="form-group">
                                            <label class="mb-1" for="desc">Mündəricat</label>
                                            <textarea placeholder="Mündəricat Daxil Edin" id="editor" name="desc[{{ $key }}]"
                                                class="mt-3 mb-3 form-control">{{ old('desc' . $key, $missium->getTranslation('desc', $key)) }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
