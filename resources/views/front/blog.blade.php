    ...
    <div class="cs-page_heading cs-style1 cs-center text-center cs-bg cs-bg-page">
        <div class="container">
            <div class="cs-page_heading_in">
                <h1 class="cs-page_title cs-font_50 cs-white_color">@lang('website.blogus')</h1>
                <ol class="breadcrumb text-uppercase">
                    <li class="breadcrumb-item">
                        <a href="{{ route('front.home') }}">@lang('website.home')</a>
                    </li>
                    <li class="breadcrumb-item active">@lang('website.blogus')</li>
                </ol>
            </div>
        </div>
    </div>
            ...
            <div class="row">
                <div class="col-xl-8 col-lg-8 col-md-12 col-12" style="margin-bottom: 30px;">
                    <div class="cs-sidebar_item widget_tag_cloud">
                        ...
                        <div class="tagcloud">
                            @foreach ($tags as $tag)
                            <a href="{{ route('front.blogs', $tag->slug) }}" class="tag-cloud-link">{!!$tag->name!!}</a>
                            @endforeach
                        </div>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-12 col-12" style="margin-bottom: 30px;">
                    <div class="cs-sidebar_item widget_search">
                        ...
                        <form class="cs-sidebar_search" action="{{route('front.blogsearch')}}" method="POST">
                            @csrf
                ...
                @foreach ($blogs as $blog)
                    <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6 col-12" style="margin-bottom: 30px;">
                        <div class="cs-post cs-style2" data-aos="fade-up">
                            <a href="{{route('front.blogDetails',['id'=>$blog->id,'slug'=>$blog->slug])}}" class="cs-post_thumb cs-radius_15">
                                <img src="{{ $blog->image }}" alt="Post" class="w-100 cs-radius_15"
                                    style="height: 450px; object-fit: cover;">
                            </a>
                            <div class="cs-post_info" data-aos="fade-right">

                                <h2 class="cs-post_title">
                                    <a href="{{route('front.blogDetails',['id'=>$blog->id,'slug'=>$blog->slug])}}">{{ $blog->title }}</a>
                                </h2>
                                <div class="cs-post_sub_title">{!! substr($blog->desc, 0, 150) !!}</div>
                                <a href="{{route('front.blogDetails',['id'=>$blog->id,'slug'=>$blog->slug])}}" class="cs-text_btn">
